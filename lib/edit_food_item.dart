...
class _EditFoodItemScreenState extends State<EditFoodItemScreen> {
  ...
  void initState() {
    ...
    // Setup the Firebase reference for the food item
    _foodRef = FirebaseDatabase.instance.ref("restaurant")
  ...
  Future<void> _saveChanges() async {
    ...
    if (_imageFile != null) {
      final uuid = Uuid().v4();
      final storageRef = FirebaseStorage.instance.ref("restaurant_menu_images/${widget.restaurantId}/$uuid.jpg");
      await storageRef.putFile(_imageFile!);
      _imageUrl = await storageRef.getDownloadURL();
    }

    // Update food item data in Firebase
    ...
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Food item updated successfully")));
